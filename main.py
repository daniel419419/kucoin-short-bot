...
def check_short_signal(df):
    ...
    if adx_now > 20 and c < ema_now and prev_c > ema_prev:
        return {'entry': c, 'sl': c + atr_now, 'tp': c - atr_now, 'atr': atr_now}
    return None
        ...
        for symbol in symbols:
            ...
            if symbol in active_trades:
                ...
                if price <= trade['tp']:
                    ...
                    bot.send_message(chat_id=chat_id, text=f"✅ TP HIT {symbol} @ {price}\nBalance: ${balance:.2f}")
                    del active_trades[symbol]
                elif price >= trade['sl']:
                    balance -= RISK_PER_TRADE
                    bot.send_message(chat_id=chat_id, text=f"❌ SL HIT {symbol} @ {price}\nBalance: ${balance:.2f}")
                    del active_trades[symbol]
                continue

            signal = check_short_signal(df)
            if signal:
                active_trades[symbol] = {**signal, 'direction': 'short'}
